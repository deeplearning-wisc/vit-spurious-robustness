...
cub_dir = r'../waterbirds/CUB_200_2011'
places_dir = r'../waterbirds/places365_standard/data_256'
output_dir = r'../waterbirds/'

#print(os.listdir(r'../waterbirds/CUB_200_2011'))
...
#print(place_filenames)
output_subfolder = os.path.join(output_dir, dataset_name)
os.makedirs(output_subfolder, exist_ok=True)

df.to_csv(os.path.join(output_subfolder, 'metadata.csv'))

output_dir_1 = r'../waterbird_land_new/'
output_folder_1 = os.path.join(output_dir_1, "images")
#os.makedirs(output_folder_1, exist_ok=True)

output_dir_2 = r'../waterbird_water/'
output_folder_2 = os.path.join(output_dir_2, "images")
...
for ind, i in enumerate(df.index):
    ...
    # Skip front /
  #  print(df.loc[i, 'place_filename'][1:])
   # place_path = os.path.join(places_dir,place_filenames[i])
    place_path = places_dir + place_filenames[i]
 #   place_path = os.path.join(places_dir, df.loc[i, 'place_filename'][1:])
    #place_path_1 = os.path.join(places_dir, df.loc[i+1, 'place_filename'][1:])

    place = Image.open(place_path).convert('RGB')
   # place_1 = Image.open(place_path_1).convert('RGB')

    img_black = Image.fromarray(np.around(img_np * seg_np).astype(np.uint8))
    combined_img = combine_and_mask(place, seg_np, img_black)
    output_path_1 = os.path.join(output_folder_1, df.loc[i, 'img_filename'])
   # combined_img_1 = combine_and_mask(place_1, seg_np, img_black)
	
  #  output_path_1 = os.path.join(output_folder_1, df.loc[i, 'img_filename'].split('/')[0], f"{j}.jpg")
  #  print(output_path_1)
  #  output_path_2 = os.path.join(output_folder_2, df.loc[i, 'img_filename'].split('/')[0], f"{j}.jpg")
    os.makedirs('/'.join(output_path_1.split('/')[:-1]), exist_ok=True)
  #  os.makedirs('/'.join(output_path_2.split('/')[:-1]), exist_ok=True)
   

    combined_img.save(output_path_1)
  #  combined_img_1.save(output_path_2)
  #  j = j +1
 #   break
    if ind == 5000: break
